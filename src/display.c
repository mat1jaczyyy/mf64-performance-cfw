...
uint8_t g_display_buffer[64 * 2 * 3]; // 64 Buttons * 2 LEDs * 3 RGB-Werte
...
void draw_animation(uint8_t* buffer, uint8_t start, uint8_t pos) {
	...
	for (uint8_t i = 0; i < max; i++) {
		// Mit dem neuen Layout: jeder Button hat 6 Bytes (2 LEDs × 3 RGB)
		uint8_t button_index = start + offset + i * 3;
		// Beide LEDs des Buttons mit der Animation-Farbe setzen
		buffer[button_index * 6 + 0] = GEOMETRIC_COLOR_B; // LED 0: B
		buffer[button_index * 6 + 1] = GEOMETRIC_COLOR_R; // LED 0: R
		buffer[button_index * 6 + 2] = GEOMETRIC_COLOR_G; // LED 0: G
		buffer[button_index * 6 + 3] = GEOMETRIC_COLOR_B; // LED 1: B
		buffer[button_index * 6 + 4] = GEOMETRIC_COLOR_R; // LED 1: R
		buffer[button_index * 6 + 5] = GEOMETRIC_COLOR_G; // LED 1: G
	}
}
...
void fastrgb_state(uint8_t* buffer) {
	// Die Hardware erwartet 128 LEDs (64 Buttons × 2 LEDs pro Button)
...
void fastrgb_state_single_led(uint8_t* buffer) {
	for (uint8_t i=0; i<NUM_BUTTONS; i++) {
		...
		buffer[i * 3 + 1] = g_fastrgb_state[i][0][0]; // R
		buffer[i * 3 + 2] = g_fastrgb_state[i][0][1]; // G
	}
}
